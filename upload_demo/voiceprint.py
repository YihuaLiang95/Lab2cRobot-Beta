...
def voiceprint(filename = 'output.wav'):
    #生成训练过的模型
    names=['Yu Yang', 'Jie Lian', 'Wei Tao', 'Si Kai', 'Ze Long']
    ...
    #    name = names[i]
    #    save_path_name = './model/model'+name+'.m'
    #    joblib.dump(GMMs[i], save_path_name, protocol=2)
        
    #model loading
    GMMs=[]
    for i in range(len(names)):
        name = names[i]
        save_path_name = '/home/tbsi2c/catkin_ws/src/rikirobot_project/rikirobot/script/model/model'+name+'.m'
...
if __name__=="__main__":
    a = voiceprint(filename = 'Sample.wav')
    print(a)
