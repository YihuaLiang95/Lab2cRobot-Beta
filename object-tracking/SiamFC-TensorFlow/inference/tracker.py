...
class Tracker(object):
  ...
  def track(self, sess, first_bbox, frames, logdir='/tmp'):
    ...
    for i, filename in enumerate(frames):
      if i > 0 or include_first:  # We don't really want to process the first image unless intended to do so.
        ...
        response = outputs['response'] # 3,272,272
        ...
        # from 17x17 to 272x272, upsample factor is `16`.
  ...
  def track_frame(self, sess, frame, 
                                current_param,logdir='/tmp'):
    """Runs tracking on a single frame from a video stream."""
    # Run tracking
    current_target_state = current_param["current_target_state"]
    original_target_height = current_param["original_target_height"]
    original_target_width = current_param["original_target_width"]
    search_center = current_param["search_center"]
    ...
    outputs, metadata = self.siamese_model.inference_step_frame(sess, input_feed)
    search_scale_list = outputs['scale_xs']
    ...
    current_param["current_target_state"] = current_target_state

    return reported_bbox,current_param
